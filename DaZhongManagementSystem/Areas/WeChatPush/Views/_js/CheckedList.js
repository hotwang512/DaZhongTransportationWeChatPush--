...
var $page = function () {
    ...
    //推送按钮事件
    selector.$btnSend().on('click', function () {
        selection = [];
        ...
        if (selection.length < 1) {
            jqxNotification("请选择您要推送的数据！", null, "error");
        } else {
            WindowConfirmDialog(push, "您确定要推送选中的数据？", "确认框", "确定", "取消");
        }
    });
    ...
    function LoadTable() {
        ...
        var DraftListSource =
        {
            ...
            url: "/WeChatPush/CheckedList/GetWeChatPushListBySearch"    //获取数据源的路径
        };
    ...
    function detailFunc(row, column, value, rowData) {
        var container = "";
        container = "<a href='CheckedDetail?Vguid=" + rowData.VGUID + "&isEdit=true' style=\"text-decoration: underline;color: #333;\">" + rowData.Title + "</a>";
        return container;
    }
...
function push() {
    ...
    $.ajax({
        url: "/WeChatPush/CheckedList/PushSubmitList",
        data: { vguidList: selection },
        traditional: true,
        type: "post",
        success: function (msg) {
            if (msg.isSuccess) {
                jqxNotification("提交推送成功！", null, "success");
                selector.$grid().jqxDataTable('updateBoundData');
            }
            else {
                jqxNotification("提交推送失败！", null, "error");
            }
        }
    });
};
