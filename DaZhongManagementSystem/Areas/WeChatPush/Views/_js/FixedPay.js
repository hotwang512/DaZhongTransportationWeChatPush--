...
if (fee == "0%" || fee == "") {
    ...
    $("#spDriverPay").html("<div style='float:left;line-height:45px;'>实际付款金额包含微信手续费</div>" + span + "<div style='float:left;line-height:45px;'>!<div>");
}
...
function startWxPay(total_fee) {
    $.ajax({
        type: "POST",
        url: "/WeChatPush/WeChatRevenue/GetPaySign",
        data: { openid: $("#txtOpenId").val(), totalFee: total_fee, personVguid: $("#txtVguid").val(), pushContentVguid: $("#txtpushContentVguid").val(), revenueType: 1 },
        beforeSend: function () {
