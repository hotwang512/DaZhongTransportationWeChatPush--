...
namespace DaZhongManagementSystem.Areas.WeChatPush.Controllers.CheckedList.BusinessLogic
{
    public class CheckedListLogic
    {
        public CheckedServer _cs;
        public CheckedListLogic()
        {
            _cs = new CheckedServer();
        }
        ...
        /// <returns></returns>
        public bool PushSubmitList(string[] vguidList)
        {
            bool result = false;
            foreach (var item in vguidList)
            {
                result = _cs.PushSubmitList(item);
            }
            return result;
        }
