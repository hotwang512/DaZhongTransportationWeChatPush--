...
$(function () {
    $.ajax({
        url: "/BasicDataManagement/WeChatQuestion/GetQuestionAll",
        data: {
            vguid: $("#QuestionVguid").val(),//"11F62166-64A8-4B05-9B97-F534C804181F",
        ...
        success: function (data) {
            ...
            if (dataDetailRow[0].Status == "0" && $("#isHistory").val() == "0") {
                if ($("#txtIsValid").val() == "True") {
                    $("#errorQuestion").css("display", "block");
                    return false;
                }
            }
            ...
            if (dataDetailRow[0].Status == "2" && $("#isHistory").val() == "0") {
                ...
                for (var i = 0; i < dataLen; i++) {
                    //判断是否有简答题（QuestionnaireType等于4代表有简答题）
                    if (dataDetailRow[i].QuestionnaireType == 4) {
                        $("#success").show();
                        return false;
                    }
                }
            ...
            MainRowID = data.MainRow[0].QuestionnaireVguid; //当前问卷的套题ID
            for (var i = 0; i < len; i++) {
                var htmler1 = ""; //打印题目信息（选项和答案）
                var questions = ''; //显示在每个页面的内容
                var oldAnswer = dataDetailRow[i].Answer;
                if (oldAnswer == null) {
                    oldAnswer = "";
                }
                ...
                var test = dataDetailRow[i].QuestionnaireType; //判断当前题型
                //console.log(test);
                //通过题型判断生成指定的页面
                if (test == 1) {
                    var itemstr = dataDetailRow[i].QuestionOption.split(",|");
                    ...
                    questions = '<div class="swiper-slide swiper-no-swiping"><div class="swiper-slide-item">' +
						'<div class="s_title">' +
						'<div class="st_title">' + '<span>[单选题]&nbsp&nbsp</span>' + dataDetailRow[i].QuestionnaireName + '</div>' +
						'</div>' +
                    ...
                    questions = '<div class="swiper-slide swiper-no-swiping"><div  class="swiper-slide-item">' +
						'<div class="s_title">' +
						'<div class="st_title">' + '<span>[多选题]&nbsp&nbsp</span>' + dataDetailRow[i].QuestionnaireName + '</div>' +
						'</div>' +
						'<div class="s_option">' + htmler1 + '</div>' +
						'</div></div>';
                } else if (test == 3) {
                    ...
                    questions = '<div class="swiper-slide swiper-no-swiping"><div class="swiper-slide-item">' +
						'<div class="s_title">' +
						'<div class="st_title">' + '<span>[判断题]&nbsp&nbsp</span>' + dataDetailRow[i].QuestionnaireName + '</div>' +
						'</div>' +
						'<div class="s_option">' + htmler1 + '</div>' +
						'</div></div>';
                } else if (test == 4) {
                    questions = '<div class="swiper-slide swiper-no-swiping"><div class="swiper-slide-item">' +
						'<div class="s_title">' +
						'<div class="st_title">' + '<span>[简答题]&nbsp&nbsp</span>' + dataDetailRow[i].QuestionnaireName + '</div>' +
						'</div>' +
                ...
                $(".swiper-wrapper").append(questions);
            ...
            if ($("#isHistory").val() == "1") {
                ...
                $.ajax({
                    url: "/BasicDataManagement/WeChatQuestion/IsPushed",
                    data: {
                    ...
                    success: function (msg) {
                        switch (msg.respnseInfo) {
                            case "0":
                                $(".f_center2").text("获取问卷");
                                $(".f_center1").addClass("active").css("background", "rgb(20, 108, 127)");
                                break;
                            case "1":
                                isPushed = 1;
            ...
            $(".option").click(function () {
                ...
                if ($(this).attr("nameSub") == 1) {
                    ...
                    console.log(goods);
                    var id = dataDetailRow[count].QuestionnaireDetailVguid; // 每一道题的ID
                    console.log(id);
                    ...
                    //给后台上传数据
                    $.ajax({
                        url: "/BasicDataManagement/WeChatQuestion/SubmitUserAnswer",
                        data: {
                            BusinessPersonnelVguid: $("#PersonVguid").val(),//"B3A5A687-2CB8-4693-B659-1C1EDCE13B14",
                            Answer: Array,
                            BusinessQuestionnaireVguid: MainRowID,
                            BusinessQuestionnaireDetailVguid: id
                        },
                        type: "POST",
                        success: function (data) {
                            console.log(data);
...
function LeaveSubmit(e) {
    ...
    if (test != "") {
        var id = dataDetailRow[count].QuestionnaireDetailVguid;
        $(".ss").eq(count).addClass("active").css("background", "rgb(20, 108, 127)");
        //判断那个答案选项被追加上样式
        number1++;
        //判断答题是否已经完成
...
$(".f_center").click(function () {
    var url = $("#OpenHttpAddress").val() + "/BasicDataManagement/WeChatQuestion/Index?questionVguid=" + MainRowID;
    if ($("#isHistory").val() == "1") {
        $.ajax({
            url: "/BasicDataManagement/WeChatQuestion/ReWechatPushQuestion",
            data: {
                BusinessPersonnelVguid: $("#PersonVguid").val(),
                wechatMainVguid: $("#txtWechatMain").val()
            },
        ...
        $.ajax({
            url: "/BasicDataManagement/WeChatQuestion/SubmitAllQuestion",
            data: {
                BusinessPersonnelVguid: $("#PersonVguid").val(),
                businessQuestionVguid: MainRowID
            },
            type: "POST",
            success: function (data) {
                if (data.isSuccess == true) {
                    //若是问卷推送(null)，答题完后刷新当前问卷界面即可
...
$(function () {
    ...
    $(".upload").click(function () {
        if ($("#file").val() != "" && $("#file").val() != null) {
            ...
            $("#photo").ajaxSubmit({
                url: "/BasicDataManagement/WeChatQuestion/LoadUserPic",
                data: {
                    personVguid: $("#PersonVguid").val(),//"B3A5A687-2CB8-4693-B659-1C1EDCE13B14",
                    questionVguid: MainRowID
                },
                type: "post",
                dataType: 'json',
                success: function (data) {
